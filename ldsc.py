...
def ldscore(args, log):
    ...
    if args.annot is not None:  # read --annot
        try:
            ...
            log.log(f"Error parsing .annot file {args.annot}")
            raise
    ...
    df = pd.DataFrame(
        np.concatenate([geno_array.df, lN.astype(float)], axis=1), columns=new_colnames
    )
    ...
    if annot_matrix is not None:
        ...
        M_5_50 = np.atleast_1d(
            np.squeeze(np.asarray(np.sum(annot_matrix[ii, :] != 0, axis=0)))
        )
    else:
        M = [geno_array.m]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # summary statistics
            ...
            and (args.ref_ld or args.ref_ld_chr or args.ref_ld_chr_cts)
            ...
            if args.overlap_annot and not args.not_M_5_50:
                if not (
                    ...
                    or (args.frqfile_chr and args.ref_ld_chr_cts)
                ):
                    raise ValueError(
