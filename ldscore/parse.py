...
def read_csv(fh, **kwargs):
    try:
        tbl = pd.read_csv(fh, sep="\s+", na_values=".", engine="python", **kwargs)
        return tbl
    except Exception as e:
        print(f"Error parsing {fh}")
        raise e
...
def l2_parser(fh, compression, column_name=None):
    ...
    x = read_csv(
        ...
        usecols=["CHR", "SNP", "BP", column_name] if column_name else None,
    )
    if (
...
def ldscore(fh, num=None, column_name=None):
    ...
    if num is not None:  # num files, e.g., one per chromosome
        ...
        s, compression = which_compression(first_fh)
        chr_ld = [
            l2_parser(sub_chr(fh, i) + suffix + s, compression, column_name=column_name)
            for i in chrs
        ]
        x = pd.concat(chr_ld)  # automatically sorted by chromosome
    else:  # just one file
...
def M(fh, num=None, N=2, common=False, sub_idx=None):
    ...
    if num is not None:
        if sub_idx is not None:
            x = np.sum(
                [
                    parsefunc(sub_chr(fh, i) + suffix)[sub_idx]
...
def annot(fh_list, num=None, frqfile=None, return_annot_only=False):
    """
    ...
    if num is not None:  # 22 files, one for each chromosome
        ...
        for chrom in chrs:
            ...
            annot_matrix_chrs.append(annot_matrix_chr)
            M_tot += len(df_annot_chr_list[0])
        x = sum(y)
        if return_annot_only:
            return np.vstack(annot_matrix_chrs), M_tot
    else:  # just one file
        for i, fh in enumerate(fh_list):
    ...
    class IDContainer(object):
        ...
        def __read__(self, fname):
            ...
            if fname.endswith(".npz"):
                ...
                self.df = pd.DataFrame.sparse.from_spmatrix(
                    scipy.sparse.load_npz(fname)
                )
            else:
                self.df = pd.read_csv(
                    fname,
