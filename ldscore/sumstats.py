...
def _read_annot_cts(args, log):
    ...
    annot, M_tot = _read_chr_split_files(
        ...
        log,
        "annot matrix",
        partial(ps.annot, return_annot_only=True),
        frqfile=args.frqfile,
    )
    cts_annot, M_cts = _read_chr_split_files(
        args.ref_ld_chr_cts,
        None,
        log,
        "cts annot matrix",
    ...
    all_annots = np.concatenate([cts_annot, annot], axis=1)
    return np.dot(all_annots.T, all_annots), M_tot
...
def cell_type_specific(args, log):
    ...
    if args.n_annot is None:
        for name, ct_ld_chr in [
            x.split() for x in open(args.ref_ld_chr_cts).readlines()
        ]:
            ...
            ref_ld_cts = np.array(
                pd.merge(keep_snps, ref_ld_cts_allsnps, on="SNP", how="left").iloc[
                    :, 1:
                ]
            )
        ...
        for annot_id in tqdm(range(args.n_annot)):
            ...
            ref_ld_cts_allsnps = _read_chr_split_files(
                ...
                log,
                "cts reference panel LD Score",
                partial(ps.ldscore_fromlist, column_name=name),
            )
            log.log("Performing regression.")
            ref_ld_cts = np.array(
                pd.merge(keep_snps, ref_ld_cts_allsnps, on="SNP", how="left").iloc[
            ...
            # if args.overlap_annot:
            #     cts_and_all_idx = [annot_id] + list(range(args.n_annot, args.n_annot + len(M_annot_all_regr)))
            #     _overlap_matrix = overlap_matrix[np.ix_(cts_and_all_idx, cts_and_all_idx)]
            #     res_df = hsqhat._overlap_output([name] + ref_ld_cnames_all_regr.tolist(), _overlap_matrix, M_annot, M_tot, args.print_coefficients)
            #     full_res_df.append(res_df)
            coef, coef_se = hsqhat.coef[0], hsqhat.coef_se[0]
        ...
        # if args.overlap_annot:
        #     full_res_df = pd.concat(full_res_df, axis=0)
        #     full_res_df.to_csv(args.out + ".cell_type_full_results.txt", sep="\t", index=False)
...
def estimate_h2(args, log):
    ...
    # pkl.dump({'M_annot':M_annot, 'ref_ld':ref_ld,'sumstats':sumstats, "hsqhat":hsqhat}, open(f"{args.out}.tmp.pkl", 'wb'))
    log.log(
        hsqhat.summary(
    ...
    # Save coefficients covariance matrix
    np.save(f"{args.out}.coef_cov", hsqhat.coef_cov)
    return hsqhat
